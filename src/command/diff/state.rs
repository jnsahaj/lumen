    ...
    pub fn new(file_diffs: Vec<FileDiff>, focus_file: Option<&str>) -> Self {
        ...
        let (sidebar_selected, current_file) = if let Some(focus_path) = focus_file {
            if let Some(file_idx) = file_diffs.iter().position(|f| f.filename == focus_path) {
                let sidebar_idx = sidebar_items
                    .iter()
                    .position(|item| {
                        matches!(item, SidebarItem::File { file_index, .. } if *file_index == file_idx)
                    })
                    .unwrap_or(0);
                ...
                Self::find_first_file(&sidebar_items)
            }
        } else {
            Self::find_first_file(&sidebar_items)
        };
...
mod tests {
    ...
    fn test_focus_updates_sidebar_selection() {
        ...
        if let SidebarItem::File { file_index, .. } = &state.sidebar_items[state.sidebar_selected] {
